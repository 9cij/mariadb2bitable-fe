...
const fetchUserData = async () => {
  try {
    ...
    if (response.data.code == 1) {
      ...
      sqlquery.value = userData.SQLQuery;
    }
  } catch (error) {
...
const onSubmit = async () => {
  ...
  try {
    ...
    if (response.data) {
      ...
      gotopath(response.data, form.value.input1, form.value.input2, form.value.username, form.value.password, form.value.loginAccount, form.value.loginPassword);
    } else {
